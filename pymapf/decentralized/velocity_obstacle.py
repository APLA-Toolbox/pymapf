...
class MultiAgentVelocityObstacle:
    def __init__(self, simulation_time=8.0, timestep=.1, log_level="WARNING"):
    ...
    def register_agent(self, id, start, goal, radius=.5, vmax=2, vmin=.2):
        if id in self.agents:
            logging.warning("Failed to register agent: ID has already been registered.")
            return
        agent = VelocityAgent(id, start, goal, self.number_of_timesteps, self.timestep, radius, vmax, vmin)
